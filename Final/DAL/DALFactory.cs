...
namespace DALAPI
{
    public static class DALFactory
    {
        ...
        public static IDAL GetDAL()
        {
            ...
            DALConfig.DALPac DALPac;
            try // get dal package info according to <dal> element value in config file
            {
                DALPac = DALConfig.DALPacs[dlType];
            }
            catch (KeyNotFoundException ex)
            {
                // if package name is not found in the list - there is a problem in config.xml
                throw new DALConEx($"Wrong DL type: {dlType}", ex);
            }
            string DALPacName = DALPac.PkgName;
            string dlNameSpace = DALPac.NameSpace;
            string dlClass = DALPac.ClassName;

            try // Load into CLR the dal implementation assembly according to dll file name (taken above)
            {
                Assembly.Load(DALPacName);
            }
            catch (Exception ex)
            {
                throw new DALConEx($"Failed loading {DALPacName}.dll", ex);
            }
            ...
            try
            {
                type = Type.GetType($"{dlNameSpace}.{dlClass}, {DALPacName}", true);
            }
            catch (Exception ex)
            { // If the type is not found - the implementation is not correct - it looks like the class name is wrong...
                throw new DALConEx($"Class not found due to a wrong namespace or/and class name: {DALPacName}:{dlNameSpace}.{dlClass}", ex);
            }
            ...
            try
            {
                ...
                if (dal == null)
                    throw new DALConEx($"Class {dlNameSpace}.{dlClass} instance is not initialized");
                // now it looks like we have appropriate dal implementation instance :-)
                return dal;
            }
            catch (NullReferenceException ex)
            {
                throw new DALConEx($"Class {dlNameSpace}.{dlClass} is not a singleton", ex);
            }
